...
const doSummaryBillReport = function(hospitalId, key) {
  return new Promise(async function(resolve, reject) {
    ...
    const caseContents = await db.cases.findAll({attributes: ['id', 'createdAt', 'updatedAt', 'Case_UploadedAt', 'Case_ScanPart', 'Case_RadiologistId', 'Case_OrthancStudyID', 'hospitalId'], include: caseInclude, where: [casewhereClous], order: orderby});
    let finalSumaryRows = [];
