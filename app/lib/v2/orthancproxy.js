	...
	return new Promise(async function(resolve, reject) {
		...
		*/
		...
		if (rqParam.method.toLowerCase() == 'get') {
			res = await request.get(orthacUrlReq);
			resolve({status: {code: 200}, res: res});
		} else if (rqParam.method.toLowerCase() == 'post') {
			res = await request.post(orthacUrlReq, rqBody, rqParam.options);
			resolve({status: {code: 200}, res: res});
		} else {
...
app.post('/preview/(:instanceID)', function(req, res) {
	...
	runcommand(command).then((stdout) => {
		...
		let link = '/' + rootname + process.env.USRPREVIEW_PATH + '/' + previewFileName				/* 'Authorization': 'Bearer Y9OoNebK+kdDnb3vHQE/jUkKHbelE23So7HZ3PacUwb4YzwJZi+0iriPjj7lRw5hYR6B8y65kqgsZS2143lHC7CTlUY7fZZfXMgAfqf5DdDlivD49f42embGQaM/pv68YH6Aq3L7lzaTjC3HdqNWaAdB04t89/1O/w1cDnyilFU=' */
			},
			body: rqBody
		}, (err, res, body) => {
			if (!err) {
