...
(function ( $ ) {
  $.fn.controlpage = function( options ) {
    const optionValues = [1, 2, 3, 4, 5, 10, 20, 30, 50, 100, 150, 200];
    ...
    const doResetButtonCmd = function(wrapper){
      ...
      $(wrapper).find('#TotalPageText').text(' / ' + settings.totalPage).css({'font-size': '20px', 'font-weight': 'bold'});
      $(wrapper).find('#ItemperPage').val(settings.itemperPage);
    }

    const calTotalPage = function(){
      let totalPage = (Number(settings.totalItem) / Number(settings.itemperPage));
      totalPage = parseInt(totalPage);
      let modPage = (Number(settings.totalItem) % Number(settings.itemperPage));
      if (modPage > 0) {
        totalPage += 1;
      }
    ...
    const setupItemperPage = function(value) {
      if (Number(value) == 0) {
        ...
        settings.itemperPage = Number(value);
      }
      calTotalPage();
    }

    const setupTotalItem = function(value) {
      settings.totalItem = Number(value);
      calTotalPage();
    }
    ...
    const doCreateItemperPageOption = function(changeEvt){
      let itemperPageSelector = $('<select id="ItemperPage"></select>');
    ...
    const init = function() {
      const itemperPageChange = function(newValue) {
        settings.itemperPage = Number(newValue);
        settings.currentPage = 1;
        goToPage(settings.currentPage);
      }
      const goToPage = function(page) {
        let itemPerPage = Number(settings.itemperPage);
        let fromItem = (Number(page-1) * itemPerPage);
        let toItem = (fromItem + itemPerPage) -1;
        let showItem = {fromItem: fromItem, toItem: toItem, toPage: Number(page), perPage: itemPerPage };
