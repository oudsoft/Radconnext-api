  ...
  const doShowAddGooditemForm = function(shopData, successCallback, cancelCallback){
	...
	const doShowGroupFilter = function(menugroups, successCallback) {
		...
		menugroups.forEach((group, i) => {
			let groupImage = $('<img/>').attr('src', group.GroupPicture).css({'width': '100%', 'height': 'auto', 'position': 'relative', 'display': 'inline'});
			let imageBox = $('<div></div>').css({'width': '110px', 'text-align': 'center'}).append($(groupImage));
