  ...
  $.fn.imagecrop = function( options ) {
    ...
    const doCreateImageTools = function(fileURL){
      ...
      let imageSrc = doCreateSourceImage(fileURL, ()=>{
        $(imageSrc).css({'position': 'relative', 'cursor': 'crosshair'});
    ...
    const doCreateSourceImage = function(imageUrl, callback) {
      ...
      srcImg.onload = function(x) {
        ...
        console.log('loaded!!' + JSON.stringify(x));
        if (callback) {
          callback();
        }
      }
    ...
    const doCreateWHInputBox = function(inputCallback, zoomCallback){
      ...
      let zoomValueInput = $('<input type="number" id="ZoomValueInput"/>').val((settings.scale * 100).toFixed(2)).css({'display': 'inline-block', 'width': '80px', 'margin-left': '10px'});
